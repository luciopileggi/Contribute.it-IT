...
Il miglior metodo per includere codice è l'inclusione di frammenti di codice da un esempio funzionante. Creare l'esempio in base alle istruzioni fornite nell'articolo [Contributing to .NET docs](dotnet-contribute.md#contributing-to-samples) (Contributo ai documenti .NET). Le regole di base per l'inclusione di codice si trovano nelle indicazioni generali per il [codice](../code-in-docs.md).
...
* `<name>` (*facoltativo*)
  * Nome del frammento di codice. Non ha alcun impatto sul codice HTML di output, ma può essere usato per rendere più leggibile il codice sorgente Markdown.
